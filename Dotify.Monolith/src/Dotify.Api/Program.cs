...
{
    options.CustomSchemaIds(x => x.GetCustomAttributes(false).OfType<DisplayNameAttribute>().FirstOrDefault()?.DisplayName
                                  ?? (x.Name.EndsWith("Dto") ? x.Name.Replace("Dto", string.Empty) : x.Name));
